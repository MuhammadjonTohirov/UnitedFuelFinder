...
/**
 ...
 use the TMGLShapeOfflineRegion class instead.
 
 ### Example
 ```swift
 let northeast = CLLocationCoordinate2D(latitude: 40.989329, longitude: -102.062592)
 let southwest = CLLocationCoordinate2D(latitude: 36.986207, longitude: -109.049896)
 let bbox = TMGLCoordinateBounds(sw: southwest, ne: northeast)
 
 let region = TMGLTilePyramidOfflineRegion(styleURL: TMGLStyle.lightStyleURL, bounds: bbox, fromZoomLevel: 11, toZoomLevel: 14)
 ...
 */
TMGL_EXPORT
@interface TMGLTilePyramidOfflineRegion : NSObject <TMGLOfflineRegion, NSSecureCoding, NSCopying>

/**
 The coordinate bounds for the geographic region covered by the downloaded
 tiles.
 */
@property (nonatomic, readonly) TMGLCoordinateBounds bounds;
...
/**
 ...
 This is the designated initializer for `TMGLTilePyramidOfflineRegion`.
 ...
 @param maximumZoomLevel The maximum zoom level to be covered by the downloaded
    ...
    level.
 */
- (instancetype)initWithStyleURL:(nullable NSURL *)styleURL bounds:(TMGLCoordinateBounds)bounds fromZoomLevel:(double)minimumZoomLevel toZoomLevel:(double)maximumZoomLevel NS_DESIGNATED_INITIALIZER;
