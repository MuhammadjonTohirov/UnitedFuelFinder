...
@class TMGLAnnotationView;
@class TMGLAnnotationImage;
@class TMGLUserLocation;
@class TMGLPolyline;
@class TMGLPolygon;
@class TMGLShape;

@protocol TMGLMapViewDelegate;
@protocol TMGLAnnotation;
@protocol TMGLOverlay;
@protocol TMGLCalloutView;
@protocol TMGLFeature;
@protocol TMGLLocationManager;

/** Options for `TMGLMapView.decelerationRate`. */
typedef CGFloat TMGLMapViewDecelerationRate NS_TYPED_EXTENSIBLE_ENUM;

/** The default deceleration rate for a map view. */
FOUNDATION_EXTERN TMGL_EXPORT const TMGLMapViewDecelerationRate TMGLMapViewDecelerationRateNormal;

/** A fast deceleration rate for a map view. */
FOUNDATION_EXTERN TMGL_EXPORT const TMGLMapViewDecelerationRate TMGLMapViewDecelerationRateFast;

/** Disables deceleration in a map view. */
FOUNDATION_EXTERN TMGL_EXPORT const TMGLMapViewDecelerationRate TMGLMapViewDecelerationRateImmediate;

/**
 The vertical alignment of an annotation within a map view. Used with
 `TMGLMapView.userLocationVerticalAlignment`.
 */
typedef NS_ENUM(NSUInteger, TMGLAnnotationVerticalAlignment) {
    /** Aligns the annotation vertically in the center of the map view. */
    TMGLAnnotationVerticalAlignmentCenter = 0,
    /** Aligns the annotation vertically at the top of the map view. */
    TMGLAnnotationVerticalAlignmentTop,
    /** Aligns the annotation vertically at the bottom of the map view. */
    TMGLAnnotationVerticalAlignmentBottom,
};

/**
 The position of scale bar, compass, logo and attribution in a map view. Used with
 `TMGLMapView.scaleBarPosition`,
 `TMGLMapView.compassViewPosition`,
 `TMGLMapView.logoViewPosition`,
 `TMGLMapView.attributionButtonPosition`.
 */
typedef NS_ENUM(NSUInteger, TMGLOrnamentPosition) {
    /** Place the ornament in the top left of the map view. */
    TMGLOrnamentPositionTopLeft = 0,
    /** Place the ornament in the top right of the map view. */
    TMGLOrnamentPositionTopRight,
    /** Place the ornament in the bottom left of the map view. */
    TMGLOrnamentPositionBottomLeft,
    /** Place the ornament in the bottom right of the map view. */
    TMGLOrnamentPositionBottomRight,
};

/**
 The mode used to track the user location on the map. Used with
 `TMGLMapView.userTrackingMode`.
 ...
 */
typedef NS_ENUM(NSUInteger, TMGLUserTrackingMode) {
    /** The map does not follow the user location. */
    TMGLUserTrackingModeNone              = 0,
    /** The map follows the user location. This tracking mode falls back
        to `TMGLUserTrackingModeNone` if the user pans the map view. */
    TMGLUserTrackingModeFollow,
    /**
        ...
        view, this tracking mode will fall back to `TMGLUserTrackingModeFollow`.
     */
    TMGLUserTrackingModeFollowWithHeading,
    /**
        ...
        this tracking mode will fall back to `TMGLUserTrackingModeFollow`.
     */
    TMGLUserTrackingModeFollowWithCourse,
};

typedef NS_ENUM(NSUInteger, TMGLPanScrollingMode) {
    /** The map allows the user to only scroll horizontally. */
    TMGLPanScrollingModeHorizontal               = 0,
    /** The map allows the user to only scroll vertically. */
    TMGLPanScrollingModeVertical,
    /** The map allows the user to scroll both horizontally and vertically. */
    TMGLPanScrollingModeDefault
};

/** Options for `TMGLMapView.preferredFramesPerSecond`. */
typedef NSInteger TMGLMapViewPreferredFramesPerSecond NS_TYPED_EXTENSIBLE_ENUM;

/**
 The default frame rate. This can be either 30 FPS or 60 FPS, depending on
 device capabilities.
 */
FOUNDATION_EXTERN TMGL_EXPORT const TMGLMapViewPreferredFramesPerSecond TMGLMapViewPreferredFramesPerSecondDefault;

/** A conservative frame rate; typically 30 FPS. */
FOUNDATION_EXTERN TMGL_EXPORT const TMGLMapViewPreferredFramesPerSecond TMGLMapViewPreferredFramesPerSecondLowPower;

/** The maximum supported frame rate; typically 60 FPS. */
FOUNDATION_EXTERN TMGL_EXPORT const TMGLMapViewPreferredFramesPerSecond TMGLMapViewPreferredFramesPerSecondMaximum;

FOUNDATION_EXTERN TMGL_EXPORT TMGLExceptionName const TMGLMissingLocationServicesUsageDescriptionException;
FOUNDATION_EXTERN TMGL_EXPORT TMGLExceptionName const TMGLUserLocationAnnotationTypeException;

/**
 An interactive, customizable map view with an interface similar to the one
 provided by Apple’s MapKit.

 Using `TMGLMapView`, you can embed the map inside a view, allow users to
 ...
 <a href="https://www.mapbox.com/mapbox-gl-style-spec/">TrimbleMaps Style Specification</a>.
 Such styles can be designed in
 <a href="https://www.mapbox.com/studio/">Mapbox Studio</a> and hosted on
 mapbox.com.

 A collection of Mapbox-hosted styles is available through the `TMGLStyle`
 ...
 Because `TMGLMapView` loads asynchronously, several delegate methods are available
 ...
 in the `TMGLMapViewDelegate` protocol documentation.

 Adding your own gesture recognizer to `TMGLMapView` will block the corresponding
 gesture recognizer built into `TMGLMapView`. To avoid conflicts, define which
 gesture takes precedence. For example, you can create your own
 `UITapGestureRecognizer` that will be invoked only if the default `TMGLMapView`
 tap gesture fails:
 ...
 Simple map view</a> example to learn how to initialize a basic `TMGLMapView`.
 */
TMGL_EXPORT
@interface TMGLMapView : UIView <TMGLStylable>
...
/**
 ...
 initialize an `TMGLMapView` with a custom style. See the
 ...
 an `TMGLMapView` with a Studio Classic style _or_ a custom style JSON. See the
 ...
 `TMGLMapView` with a third-party tile source.
 */
...
/**
 ...
 annotations.
 */
@property(nonatomic, weak, nullable) IBOutlet id<TMGLMapViewDelegate> delegate;
...
/**
 ...
 `-[TMGLMapViewDelegate mapViewDidFinishLoadingMap:]` method. It is not possible
...
/**
 The position of the scale bar. The default value is `TMGLOrnamentPositionTopLeft`.
 */
@property (nonatomic, assign) TMGLOrnamentPosition scaleBarPosition;
 ...
 */
@property (nonatomic, readonly) TMGLCompassButton *compassView;

/**
 The position of the compass view. The default value is `TMGLOrnamentPositionTopRight`.
 */
@property (nonatomic, assign) TMGLOrnamentPosition compassViewPosition;
...
/**
 The position of the logo view. The default value is `TMGLOrnamentPositionBottomLeft`.
 */
@property (nonatomic, assign) TMGLOrnamentPosition logoViewPosition;
...
/**
 ...
 `TMGLMapViewPreferredFramesPerSecondDefault`, which will adaptively set the
 preferred frame rate based on the capability of the user’s device to maintain
 a smooth experience.

 In addition to the provided `TMGLMapViewPreferredFramesPerSecond` options, this
 property can be set to arbitrary integer values.

 @see `CADisplayLink.preferredFramesPerSecond`
 */
@property (nonatomic, assign) TMGLMapViewPreferredFramesPerSecond preferredFramesPerSecond;
...
/**
 ...
 `-[TMGLMapViewDelegate mapView:didUpdateUserLocation:]` and
 `-[TMGLMapViewDelegate mapView:didFailToLocateUserWithError:]` methods.

 If setting this property to `nil` or if no custom manager is provided this
 property is set to the default location manager.

 `TMGLMapView` uses a default location manager. If you want to substitute your
 ...
 */
@property (nonatomic, null_resettable) id<TMGLLocationManager> locationManager;
 ...
 */
@property (nonatomic, readonly, nullable) TMGLUserLocation *userLocation;

/**
 The mode used to track the user location. The default value is
 `TMGLUserTrackingModeNone`.
 ...
 default user location annotation shown by `TMGLUserTrackingMode`.
 */
@property (nonatomic, assign) TMGLUserTrackingMode userTrackingMode;
 ...
 */
- (void)setUserTrackingMode:(TMGLUserTrackingMode)mode animated:(BOOL)animated __attribute__((deprecated("Use `-setUserTrackingMode:animated:completionHandler:` instead.")));
 ...
 */
- (void)setUserTrackingMode:(TMGLUserTrackingMode)mode animated:(BOOL)animated completionHandler:(nullable void (^)(void))completion;

/**
 The vertical alignment of the user location annotation within the receiver. The
 default value is `TMGLAnnotationVerticalAlignmentCenter`.

 Changing the value of this property updates the map view with an animated
 transition. If you don’t want to animate the change, use the
 `-setUserLocationVerticalAlignment:animated:` method instead.
 */
@property (nonatomic, assign) TMGLAnnotationVerticalAlignment userLocationVerticalAlignment __attribute__((deprecated("Use `-[TMGLMapViewDelegate mapViewUserLocationAnchorPoint:]` instead.")));
 ...
 */
- (void)setUserLocationVerticalAlignment:(TMGLAnnotationVerticalAlignment)alignment animated:(BOOL)animated __attribute__((deprecated("Use `-[TMGLMapViewDelegate mapViewUserLocationAnchorPoint:]` instead.")));
...
/**
 ...
 `TMGLUserTrackingModeFollowWithHeading` or
 `TMGLUserTrackingModeFollowWithCourse`.
...
/**
 ...
 value other than `TMGLUserTrackingModeFollowWithCourse`.
...
/**
 ...
 other than `TMGLUserTrackingModeFollowWithCourse`.
...
`TMGLPanScrollingModeHorizontal` only allows the user to scroll horizontally on the map,
 restricting a user's ability to scroll vertically.
`TMGLPanScrollingModeVertical` only allows the user to scroll vertically on the map,
 restricting a user's ability to scroll horizontally.
 `TMGLPanScrollingModeDefault` allows the user to scroll both horizontally and vertically
 on the map.

 By default, this property is set to `TMGLPanScrollingModeDefault`.
 */
@property (nonatomic, assign) TMGLPanScrollingMode panScrollingMode;
...
/**
 ...
 Your application can use the `TMGLMapViewDecelerationRateNormal` and
 `TMGLMapViewDecelerationRateFast` constants as reference points for reasonable
 deceleration rates. `TMGLMapViewDecelerationRateImmediate` can be used to
 ...
 */
@property (nonatomic) TMGLCoordinateBounds visibleCoordinateBounds;
 ...
 */
- (void)setVisibleCoordinateBounds:(TMGLCoordinateBounds)bounds animated:(BOOL)animated;
 ...
 */
- (void)setVisibleCoordinateBounds:(TMGLCoordinateBounds)bounds edgePadding:(UIEdgeInsets)insets animated:(BOOL)animated __attribute__((deprecated("Use `-setVisibleCoordinateBounds:edgePadding:animated:completionHandler:` instead.")));
...
/**
 ...
 @param completion The block executed after the animation finishes.
 */
- (void)setVisibleCoordinateBounds:(TMGLCoordinateBounds)bounds edgePadding:(UIEdgeInsets)insets animated:(BOOL)animated completionHandler:(nullable void (^)(void))completion;
 ...
 */
- (void)showAnnotations:(NSArray<id <TMGLAnnotation>> *)annotations animated:(BOOL)animated;
...
/**
 ...
 @param animated `YES` if you want the map region change to be animated, or `NO`
    if you want the map to display the new region immediately without animations.
 */
- (void)showAnnotations:(NSArray<id <TMGLAnnotation>> *)annotations edgePadding:(UIEdgeInsets)insets animated:(BOOL)animated __attribute__((deprecated("Use `-showAnnotations:edgePadding:animated:completionHandler:` instead.")));
...
/**
 ...
 @param completion The block executed after the animation finishes.
 */
- (void)showAnnotations:(NSArray<id <TMGLAnnotation>> *)annotations edgePadding:(UIEdgeInsets)insets animated:(BOOL)animated completionHandler:(nullable void (^)(void))completion;

/**
 A camera representing the current viewpoint of the map.
 */
@property (nonatomic, copy) TMGLMapCamera *camera;
 ...
 */
- (void)setCamera:(TMGLMapCamera *)camera animated:(BOOL)animated;
 ...
 */
- (void)setCamera:(TMGLMapCamera *)camera withDuration:(NSTimeInterval)duration animationTimingFunction:(nullable CAMediaTimingFunction *)function;
 ...
 */
- (void)setCamera:(TMGLMapCamera *)camera withDuration:(NSTimeInterval)duration animationTimingFunction:(nullable CAMediaTimingFunction *)function completionHandler:(nullable void (^)(void))completion;
...
/**
 ...
 @param completion The block to execute after the animation finishes.
 */
- (void)setCamera:(TMGLMapCamera *)camera withDuration:(NSTimeInterval)duration animationTimingFunction:(nullable CAMediaTimingFunction *)function edgePadding:(UIEdgeInsets)edgePadding completionHandler:(nullable void (^)(void))completion;
...
/**
 ...
 @param completion The block to execute after the animation finishes.
 */
- (void)flyToCamera:(TMGLMapCamera *)camera completionHandler:(nullable void (^)(void))completion;
...
/**
 ...
 @param completion The block to execute after the animation finishes.
 */
- (void)flyToCamera:(TMGLMapCamera *)camera withDuration:(NSTimeInterval)duration completionHandler:(nullable void (^)(void))completion;
...
/**
 ...
 @param completion The block to execute after the animation finishes.
 */
- (void)flyToCamera:(TMGLMapCamera *)camera withDuration:(NSTimeInterval)duration peakAltitude:(CLLocationDistance)peakAltitude completionHandler:(nullable void (^)(void))completion;
 ...
 */
- (TMGLMapCamera *)cameraThatFitsCoordinateBounds:(TMGLCoordinateBounds)bounds;
...
/**
 ...
 depending on the order of notification delivery.
 */
- (TMGLMapCamera *)cameraThatFitsCoordinateBounds:(TMGLCoordinateBounds)bounds edgePadding:(UIEdgeInsets)insets;
...
/**
 ...
 depending on the order of notification delivery.
 */
- (TMGLMapCamera *)camera:(TMGLMapCamera *)camera fittingCoordinateBounds:(TMGLCoordinateBounds)bounds edgePadding:(UIEdgeInsets)insets;
...
/**
 ...
 depending on the order of notification delivery.
 */
- (TMGLMapCamera *)camera:(TMGLMapCamera *)camera fittingShape:(TMGLShape *)shape edgePadding:(UIEdgeInsets)insets;
...
/**
 ...
 depending on the order of notification delivery.
 */
- (TMGLMapCamera *)cameraThatFitsShape:(TMGLShape *)shape direction:(CLLocationDirection)direction edgePadding:(UIEdgeInsets)insets;
...
/**
 ...
 If the `userTrackingMode` property is not `TMGLUserTrackingModeNone`, the
...
/**
 ...
 use the map view’s property `TMGLMapView.automaticallyAdjustsContentInset`instead.
 ...
 */
- (TMGLCoordinateBounds)convertRect:(CGRect)rect toCoordinateBoundsFromView:(nullable UIView *)view;
...
/**
 ...
 @param view The view in whose coordinate system the returned rectangle should
    ...
    belong to the same window as the map view.
 */
- (CGRect)convertCoordinateBounds:(TMGLCoordinateBounds)bounds toRectToView:(nullable UIView *)view;
...
/**
 ...
 The objects in this array must adopt the `TMGLAnnotation` protocol. If no
 annotations are associated with the map view, the value of this property is
 `nil`.
 */
@property (nonatomic, readonly, nullable) NSArray<id <TMGLAnnotation>> *annotations;

/**
 Adds an annotation to the map view.

 @note `TMGLMultiPolyline`, `TMGLMultiPolygon`, `TMGLShapeCollection`, and
    `TMGLPointCollection` objects cannot be added to the map view at this time.
 ...
 @param annotation The annotation object to add to the receiver. This object
    must conform to the `TMGLAnnotation` protocol. The map view retains the
    annotation object.
 ...
 annotation to an `TMGLMapView` object.
 */
- (void)addAnnotation:(id <TMGLAnnotation>)annotation;

/**
 Adds an array of annotations to the map view.

 @note `TMGLMultiPolyline`, `TMGLMultiPolygon`, and `TMGLShapeCollection` objects
    cannot be added to the map view at this time. Nor can `TMGLMultiPoint`
    objects that are not instances of `TMGLPolyline` or `TMGLPolygon`. Any
    multipoint, multipolyline, multipolygon, or shape collection objects that
    are specified are silently ignored.

 @param annotations An array of annotation objects. Each object in the array
    must conform to the `TMGLAnnotation` protocol. The map view retains each
    individual annotation object.
 */
- (void)addAnnotations:(NSArray<id <TMGLAnnotation>> *)annotations;
...
/**
 ...
 @param annotation The annotation object to remove. This object must conform
    to the `TMGLAnnotation` protocol
 */
- (void)removeAnnotation:(id <TMGLAnnotation>)annotation;
...
/**
 ...
 @param annotations The array of annotation objects to remove. Objects in the
    array must conform to the `TMGLAnnotation` protocol.
 */
- (void)removeAnnotations:(NSArray<id <TMGLAnnotation>> *)annotations;

/**
 Returns an `TMGLAnnotationView` if the given annotation is currently associated
 with a view, otherwise nil.

 @param annotation The annotation associated with the view.
    Annotation must conform to the `TMGLAnnotation` protocol.
 */
- (nullable TMGLAnnotationView *)viewForAnnotation:(id <TMGLAnnotation>)annotation;

/**
 Returns a reusable annotation image object associated with its identifier.

 For performance reasons, you should generally reuse `TMGLAnnotationImage`
 ...
 reuse an `TMGLAnnotationImage`.
 */
- (nullable __kindof TMGLAnnotationImage *)dequeueReusableAnnotationImageWithIdentifier:(NSString *)identifier;

/**
 Returns a reusable annotation view object associated with its identifier.

 For performance reasons, you should generally reuse `TMGLAnnotationView`
 objects for identical-looking annotations in your map views. Dequeueing
 saves time and memory during performance-critical operations such as
 scrolling.
 ...
 @return An annotation view object with the given identifier, or `nil` if no
    such object exists in the reuse queue.
 */
- (nullable __kindof TMGLAnnotationView *)dequeueReusableAnnotationViewWithIdentifier:(NSString *)identifier;
 ...
 */
@property (nonatomic, readonly, nullable) NSArray<id <TMGLAnnotation>> *visibleAnnotations;
...
/**
 ...
 @return An array of objects that adopt the `TMGLAnnotation` protocol or `nil` if
    no annotations associated with the map view are currently visible in the
    rectangle.
 */
- (nullable NSArray<id <TMGLAnnotation>> *)visibleAnnotationsInRect:(CGRect)rect;
...
/**
 ...
 If the annotation is of type `TMGLPointAnnotation` and is offscreen, the camera
 ...
 selected.
 */
@property (nonatomic, copy) NSArray<id <TMGLAnnotation>> *selectedAnnotations;
...
/**
 ...
 | `YES`            | The annotation is selected, and the callout is presented. If the annotation is not visible (or is partially visible) *and* is of type `TMGLPointAnnotation`, the map is panned so that the annotation and its callout are brought into view. The annotation is *not* centered within the viewport. |
 ...
 change the camera.
 */
- (void)selectAnnotation:(id <TMGLAnnotation>)annotation animated:(BOOL)animated __attribute__((deprecated("Use `-selectAnnotation:animated:completionHandler:` instead.")));
...
/**
 ...
 change the camera.
 */
- (void)selectAnnotation:(id <TMGLAnnotation>)annotation animated:(BOOL)animated completionHandler:(nullable void (^)(void))completion;
...
/**
 ...
 @param annotation The annotation object to select.
 @param moveIntoView If the annotation is not visible (or is partially visible) *and* is of type `TMGLPointAnnotation`, the map is panned so that the annotation and its callout are brought into view. The annotation is *not* centered within the viewport.
 @param animateSelection If `YES`, the annotation's selection state and callout view's presentation are animated.
 @param completion The block executed after the animation finishes.
 */
- (void)selectAnnotation:(id <TMGLAnnotation>)annotation moveIntoView:(BOOL)moveIntoView animateSelection:(BOOL)animateSelection completionHandler:(nullable void (^)(void))completion;
 ...
 */
- (void)deselectAnnotation:(nullable id <TMGLAnnotation>)annotation animated:(BOOL)animated;

- (TMGLCompactCalloutView *)calloutViewForAnnotation:(id <TMGLAnnotation>)annotation;
...
/**
 ...
 The objects in this array must adopt the `TMGLOverlay` protocol. If no
 overlays are associated with the map view, the value of this property is
 empty array.
 */
@property (nonatomic, readonly, nonnull) NSArray<id <TMGLOverlay>> *overlays;
...
/**
 ...
 @param overlay The overlay object to add. This object must conform to the
    `TMGLOverlay` protocol. */
- (void)addOverlay:(id <TMGLOverlay>)overlay;
...
/**
 ...
 @param overlays An array of objects, each of which must conform to the
    `TMGLOverlay` protocol.
 */
- (void)addOverlays:(NSArray<id <TMGLOverlay>> *)overlays;
 ...
 */
- (void)removeOverlay:(id <TMGLOverlay>)overlay;
...
/**
 ...
 @param overlays An array of objects, each of which conforms to the `TMGLOverlay`
    protocol.
 */
- (void)removeOverlays:(NSArray<id <TMGLOverlay>> *)overlays;
...
/**
 ...
 `TMGLMapView` object for visible `TMGLFeature` objects.
 */
- (NSArray<id <TMGLFeature>> *)visibleFeaturesAtPoint:(CGPoint)point NS_SWIFT_NAME(visibleFeatures(at:));
...
/**
 ...
 @return An array of objects conforming to the `TMGLFeature` protocol that
    represent features in the sources used by the current style.
 */
- (NSArray<id <TMGLFeature>> *)visibleFeaturesAtPoint:(CGPoint)point inStyleLayersWithIdentifiers:(nullable NSSet<NSString *> *)styleLayerIdentifiers NS_SWIFT_NAME(visibleFeatures(at:styleLayerIdentifiers:));
...
/**
 ...
 `TMGLShapeSource` and `TMGLVectorTileSource` objects but does not include
 anything from `TMGLRasterTileSource` objects, or from video or canvas sources,
 ...
 `TMGLFeature` object representing that bus stop is returned and its
 ...
 visibility, use the
 `-[TMGLVectorTileSource featuresInSourceLayersWithIdentifiers:predicate:]` and
 `-[TMGLShapeSource featuresMatchingPredicate:]` methods on the relevant sources.

 The returned features may also include features corresponding to annotations.
 These features are not object-equal to the `TMGLAnnotation` objects that were
 originally added to the map. To query the map for annotations, use
 `visibleAnnotations` or `-[TMGLMapView visibleAnnotationsInRect:]`.

 @note Layer identifiers are not guaranteed to exist across styles or different
    versions of the same style. Applications that use this API must first set
    the style URL to an explicitly versioned style using a convenience method
    like `+[TMGLStyle outdoorsStyleURLWithVersion:]`, `TMGLMapView`’s “Style URL”
 ...
 @return An array of objects conforming to the `TMGLFeature` protocol that
    represent features in the sources used by the current style.
 */
- (NSArray<id <TMGLFeature>> *)visibleFeaturesAtPoint:(CGPoint)point inStyleLayersWithIdentifiers:(nullable NSSet<NSString *> *)styleLayerIdentifiers predicate:(nullable NSPredicate *)predicate NS_SWIFT_NAME(visibleFeatures(at:styleLayerIdentifiers:predicate:));
...
/**
 ...
 @return An array of objects conforming to the `TMGLFeature` protocol that
    represent features in the sources used by the current style.
 */
- (NSArray<id <TMGLFeature>> *)visibleFeaturesInRect:(CGRect)rect NS_SWIFT_NAME(visibleFeatures(in:));
...
/**
 ...
 @return An array of objects conforming to the `TMGLFeature` protocol that
    represent features in the sources used by the current style.
 */
- (NSArray<id <TMGLFeature>> *)visibleFeaturesInRect:(CGRect)rect inStyleLayersWithIdentifiers:(nullable NSSet<NSString *> *)styleLayerIdentifiers NS_SWIFT_NAME(visibleFeatures(in:styleLayerIdentifiers:));
...
/**
 ...
 array. On the other hand, if the style does include bus stops, an `TMGLFeature`
 ...
 @return An array of objects conforming to the `TMGLFeature` protocol that
    represent features in the sources used by the current style.
 */
- (NSArray<id <TMGLFeature>> *)visibleFeaturesInRect:(CGRect)rect inStyleLayersWithIdentifiers:(nullable NSSet<NSString *> *)styleLayerIdentifiers predicate:(nullable NSPredicate *)predicate NS_SWIFT_NAME(visibleFeatures(in:styleLayerIdentifiers:predicate:));
 ...
 */
@property (nonatomic) TMGLMapDebugMaskOptions debugMask;

///**
// :nodoc:
// Convenience method for subscribing to a single event. See `-[TMGLObservable
// subscribeForObserver:events:]`.
// */
//- (void)subscribeForObserver:(nonnull TMGLObserver *)observer
//                       event:(nonnull TMGLEventType)event;
@end
