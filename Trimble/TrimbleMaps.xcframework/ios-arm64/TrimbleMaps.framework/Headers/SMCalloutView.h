...
typedef NS_OPTIONS(NSUInteger, TMGLSMCalloutArrowDirection) {
    TMGLSMCalloutArrowDirectionUp = 1 << 0,
    TMGLSMCalloutArrowDirectionDown = 1 << 1,
    TMGLSMCalloutArrowDirectionAny = TMGLSMCalloutArrowDirectionUp | TMGLSMCalloutArrowDirectionDown
};

/// options for the callout present/dismiss animation
typedef NS_ENUM(NSInteger, TMGLSMCalloutAnimation) {
    /// the "bounce" animation we all know and love from @c UIAlertView
    TMGLSMCalloutAnimationBounce,
    /// a simple fade in or out
    TMGLSMCalloutAnimationFade,
    /// grow or shrink linearly, like in the iPad Calendar app
    TMGLSMCalloutAnimationStretch
};
...
extern NSTimeInterval const kTMGLSMCalloutViewRepositionDelayForUIScrollView;

@protocol TMGLSMCalloutViewDelegate;
@class TMGLSMCalloutBackgroundView;
...
@interface TMGLSMCalloutView : UIView
#else
@interface TMGLSMCalloutView : UIView <CAAnimationDelegate>
#endif

@property (nonatomic, weak, nullable) id<TMGLSMCalloutViewDelegate> delegate;
...
@property (nonatomic, assign) TMGLSMCalloutArrowDirection permittedArrowDirection;
/// The current arrow direction
@property (nonatomic, readonly) TMGLSMCalloutArrowDirection currentArrowDirection;
...
@property (nonatomic, strong) TMGLSMCalloutBackgroundView *backgroundView;
...
@property (nonatomic, assign) TMGLSMCalloutAnimation presentAnimation, dismissAnimation;
...
+ (TMGLSMCalloutView *)platformCalloutView;
...
- (CAAnimation *)animationWithType:(TMGLSMCalloutAnimation)type presenting:(BOOL)presenting;
...
/// Abstract base class
@interface TMGLSMCalloutBackgroundView : UIView
...
@interface TMGLSMCalloutMaskedBackgroundView : TMGLSMCalloutBackgroundView
@end
...
@protocol TMGLSMCalloutViewDelegate <NSObject>
@optional
...
- (BOOL)calloutViewShouldHighlight:(TMGLSMCalloutView *)calloutView;

/// Called when the callout view is clicked. Not honored by @c SMClassicCalloutView.
- (void)calloutViewClicked:(TMGLSMCalloutView *)calloutView;
