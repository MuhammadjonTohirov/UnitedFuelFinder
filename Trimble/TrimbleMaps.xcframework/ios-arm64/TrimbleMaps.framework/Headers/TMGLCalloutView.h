...
@protocol TMGLCalloutViewDelegate;
...
/**
 ...
 To receive updates from an object that conforms to the `TMGLCalloutView` protocol,
 use the optional methods available in the `TMGLCalloutViewDelegate` protocol.
 ...
 `TMGLCalloutView`.
 */
@protocol TMGLCalloutView <NSObject>

/**
 An object conforming to the `TMGLAnnotation` protocol whose details this callout
 view displays.
 */
@property (nonatomic, strong) id <TMGLAnnotation> representedObject;
...
/**
 An object conforming to the `TMGLCalloutViewDelegate` method that receives
 messages related to the callout view’s interactive subviews.
 */
@property (nonatomic, weak) id<TMGLCalloutViewDelegate> delegate;
...
/**
 ...
 `-[TMGLCalloutView presentCalloutFromRect:inView:constrainedToRect:animated:]`
...
/**
 The `TMGLCalloutViewDelegate` protocol defines a set of optional methods that
 you can use to receive messages from an object that conforms to the
 `TMGLCalloutView` protocol. The callout view uses these methods to inform the
 delegate that the user has interacted with the the callout view.
 */
@protocol TMGLCalloutViewDelegate <NSObject>
 ...
 */
- (BOOL)calloutViewShouldHighlight:(UIView<TMGLCalloutView> *)calloutView;

/**
 Tells the delegate that the callout view has been tapped.
 */
- (void)calloutViewTapped:(UIView<TMGLCalloutView> *)calloutView;

/**
 Called before the callout view appears on screen, or before the appearance
 animation will start.
 */
- (void)calloutViewWillAppear:(UIView<TMGLCalloutView> *)calloutView;

/**
 Called after the callout view appears on screen, or after the appearance
 animation is complete.
 */
- (void)calloutViewDidAppear:(UIView<TMGLCalloutView> *)calloutView;
