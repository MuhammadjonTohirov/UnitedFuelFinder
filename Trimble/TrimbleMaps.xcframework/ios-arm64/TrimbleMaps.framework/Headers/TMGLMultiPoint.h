...
/**
 The `TMGLMultiPoint` class is an abstract superclass used to define shapes
 composed of multiple vertices.

 Create instances of `TMGLPolyline` or `TMGLPolygon` in order to use 
 properties of `TMGLMultiPoint`. Do not create instances of `TMGLMultiPoint`
 ...
 Do not confuse `TMGLMultiPoint` with `TMGLPointCollection`, which represents a
 collection of related but disconnected points.
 */
TMGL_EXPORT
@interface TMGLMultiPoint : TMGLShape
...
/**
 ...
 immediately. If the shape is part of an `TMGLShapeSource` object, you must
 explicitly set the `TMGLShapeSource.shape` property in order for any style
