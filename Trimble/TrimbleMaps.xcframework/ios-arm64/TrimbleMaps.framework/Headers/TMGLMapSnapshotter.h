...
@protocol TMGLMapSnapshotterDelegate;

/**
 An overlay that is placed within a `TMGLMapSnapshot`.
 To access this object, use `-[TMGLMapSnapshotter startWithOverlayHandler:completionHandler:]`.
 */

TMGL_EXPORT
@interface TMGLMapSnapshotOverlay : NSObject
...
draw custom overlays rendered with Core Graphics.
 ...
 @param snapshotOverlay The `TMGLMapSnapshotOverlay` provided during snapshot drawing.
 */
typedef void (^TMGLMapSnapshotOverlayHandler)(TMGLMapSnapshotOverlay * snapshotOverlay);

/**
 The options to use when creating images with the `TMGLMapSnapshotter`.
 */
TMGL_EXPORT
@interface TMGLMapSnapshotOptions : NSObject <NSCopying>
 ...
 */
- (instancetype)initWithStyleURL:(nullable NSURL *)styleURL camera:(TMGLMapCamera *)camera size:(CGSize)size;
 ...
 */
@property (nonatomic) TMGLMapCamera *camera;
 ...
 */
@property (nonatomic) TMGLCoordinateBounds coordinateBounds;
 ...
 */
TMGL_EXPORT
@interface TMGLMapSnapshot : NSObject
...
/**
 ...
 @param snapshot The `TMGLMapSnapshot` that was generated or `nil` if an error
    occurred.
 @param error The error that occured or `nil` when successful.
 */
typedef void (^TMGLMapSnapshotCompletionHandler)(TMGLMapSnapshot* _Nullable snapshot, NSError* _Nullable error);

/**
 An `TMGLMapSnapshotter` generates static raster images of the map. Each snapshot
 image depicts a portion of a map defined by an `TMGLMapSnapshotOptions` object
 you provide. The snapshotter generates an `TMGLMapSnapshot` object
 asynchronously, calling `TMGLMapSnapshotterDelegate` methods if defined, then
 ...
 For an interactive map, use the `TMGLMapView` class. Both `TMGLMapSnapshotter`
 and `TMGLMapView` are compatible with offline packs managed by the
 `TMGLOfflineStorage` class.
 ...
 ```swift
 let camera = TMGLMapCamera(lookingAtCenter: CLLocationCoordinate2D(latitude: 37.7184, longitude: -122.4365), altitude: 100, pitch: 20, heading: 0)
 
 let options = TMGLMapSnapshotOptions(styleURL: TMGLStyle.satelliteStreetsStyleURL, camera: camera, size: CGSize(width: 320, height: 480))
 options.zoomLevel = 10
 
 let snapshotter = TMGLMapSnapshotter(options: options)
 ...
 `TMGLMapSnapshotter` to generate a static image based on an `TMGLMapView`
 object's style, camera, and view bounds.
 */
TMGL_EXPORT
@interface TMGLMapSnapshotter : NSObject <TMGLStylable>
 ...
 */
- (instancetype)initWithOptions:(TMGLMapSnapshotOptions *)options NS_DESIGNATED_INITIALIZER;
 ...
 */
- (void)startWithCompletionHandler:(TMGLMapSnapshotCompletionHandler)completionHandler;
 ...
 */
- (void)startWithQueue:(dispatch_queue_t)queue completionHandler:(TMGLMapSnapshotCompletionHandler)completionHandler;
...
/**
 ...
 top of the resulting `TMGLMapSnapshot`.
 ...
 @param completionHandler The block to call with a finished snapshot. The block
     is executed on the main queue.
 */
- (void)startWithOverlayHandler:(TMGLMapSnapshotOverlayHandler)overlayHandler completionHandler:(TMGLMapSnapshotCompletionHandler)completionHandler;

/**
 Cancels the snapshot creation request, if any.
 
 Once you call this method, you cannot resume the snapshot. In order to obtain
 the snapshot, create a new `TMGLMapSnapshotter` object.
 */
- (void)cancel;
 ...
 */
@property (nonatomic) TMGLMapSnapshotOptions *options;
 ...
 */
@property (nonatomic, weak) id <TMGLMapSnapshotterDelegate> delegate;

/**
 The style displayed in the resulting snapshot.
 
 Unlike the `TMGLMapSnapshotOptions.styleURL` property, this property is set to
 ...
 `-[TMGLMapSnapshotterDelegate mapSnapshotter:didFinishLoadingStyle:]` method. It
...
/**
 ...
 `TMGLMapSnapshotter` object.
 */
@protocol TMGLMapSnapshotterDelegate <NSObject>
@optional
...
*/
- (void)mapSnapshotterDidFail:(TMGLMapSnapshotter *)snapshotter withError:(NSError *)error;

/**
 Tells the delegate that the snapshotter has just finished loading a style.
 
 This method is called in response to
 `-[TMGLMapSnapshotter startWithQueue:completionHandler:]` as long as the
 `TMGLMapSnapshotter.delegate` property is set. Changes to sources or layers of
 ...
 */
- (void)mapSnapshotter:(TMGLMapSnapshotter *)snapshotter didFinishLoadingStyle:(TMGLStyle *)style;

- (void)mapSnapshotter:(TMGLMapSnapshotter *)snapshotter didFailLoadingImageNamed:(NSString *)name;
