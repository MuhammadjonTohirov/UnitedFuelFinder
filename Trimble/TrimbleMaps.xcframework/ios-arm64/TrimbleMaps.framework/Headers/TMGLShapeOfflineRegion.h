...
/**
 ...
 let triangle = TMGLPolygon(coordinates: &coordinates, count: UInt(coordinates.count))
 let region = TMGLShapeOfflineRegion(styleURL: TMGLStyle.lightStyleURL, shape: triangle, fromZoomLevel: 11, toZoomLevel: 14)
 let context = "Triangle Region".data(using: .utf8)
 TMGLOfflineStorage.shared.addPack(for: region, withContext: context!)
 ```
 
 This class requires fewer resources than TMGLTilePyramidOfflineRegion
 for irregularly shaped regions.
 */
TMGL_EXPORT
@interface TMGLShapeOfflineRegion : NSObject <TMGLOfflineRegion, NSSecureCoding, NSCopying>

/**
 The shape for the geographic region covered by the downloaded
 tiles.
 */
@property (nonatomic, readonly) TMGLShape *shape;

/**
 The minimum zoom level for which to download tiles and other resources.

 For more information about zoom levels, `-[TMGLMapView zoomLevel]`.
 */
...
/**
 ...
 This is the designated initializer for `TMGLShapeOfflineRegion`.
 ...
 @param maximumZoomLevel The maximum zoom level to be covered by the downloaded
    ...
    level.
 */
- (instancetype)initWithStyleURL:(nullable NSURL *)styleURL shape:(TMGLShape *)shape fromZoomLevel:(double)minimumZoomLevel toZoomLevel:(double)maximumZoomLevel NS_DESIGNATED_INITIALIZER;
