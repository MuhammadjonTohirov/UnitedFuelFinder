...
typedef NS_ENUM(NSUInteger, TMGLAnnotationViewDragState) {
    /**
     The view is not involved in a drag operation.
     */
    TMGLAnnotationViewDragStateNone = 0,
    /**
     ...
     */
    TMGLAnnotationViewDragStateEnding,
};

/**
 The `TMGLAnnotationView` class is responsible for marking a point annotation
 with a view. Annotation views represent an annotation object, which is an
 object that corresponds to the `TMGLAnnotation` protocol. When an annotation’s
 ...
 interactivity such as dragging, you can use an `TMGLAnnotationImage` instead to
 conserve memory and optimize drawing performance.
 */
TMGL_EXPORT
@interface TMGLAnnotationView : UIView <NSSecureCoding>
...
/**
 ...
 `-[TMGLMapView dequeueReusableAnnotationViewWithIdentifier:]` method.
...
/**
 ...
 with the new view and, in custom subclasses of `TMGLAnnotationView`, customize the view
 ...
 with views when views are provided to the map via the `-[TMGLMapViewDelegate mapView:viewForAnnotation:]`
 method.
 ...
 @return The initialized annotation view object.
 */
- (instancetype)initWithAnnotation:(nullable id<TMGLAnnotation>)annotation reuseIdentifier:(nullable NSString *)reuseIdentifier;
 ...
 */
@property (nonatomic, nullable) id <TMGLAnnotation> annotation;
...
/**
 ...
 viewpoint). This is also the behavior of `TMGLAnnotationImage` objects. When the
...
/**
 ...
 view rotates. This is also the behavior of `TMGLAnnotationImage` objects. When the
...
/**
 ...
 `TMGLAnnotationView` or `TMGLAnnotationImage` object, deselects any currently
 selected view.
...
/**
 ...
 `TMGLAnnotationView`'s behavior when it is selected.
 */
...
/**
 ...
 `TMGLAnnotationView` objects on your map.
 */
 ...
 */
@property (nonatomic, readonly) TMGLAnnotationViewDragState dragState;
 ...
 */
- (void)setDragState:(TMGLAnnotationViewDragState)dragState animated:(BOOL)animated NS_REQUIRES_SUPER;
