...
/**
 An `TMGLPolyline` object represents a shape consisting of two or more vertices,
 ...
 You can add polyline shapes to the map by adding them to an `TMGLShapeSource`
 object. Configure the appearance of an `TMGLShapeSource`’s or
 `TMGLVectorTileSource`’s polylines collectively using an `TMGLLineStyleLayer` or
 `TMGLSymbolStyleLayer` object. To access a polyline’s attributes, use an
 `TMGLPolylineFeature` object.
 ...
 `-[TMGLMapViewDelegate mapView:lineWidthForPolylineAnnotation:]`.
 ...
 `TMGLPolygon` object. To group multiple polylines together in one shape, use an
 `TMGLMultiPolyline` or `TMGLShapeCollection` object.
 ...
 let polyline = TMGLPolyline(coordinates: coordinates, count: UInt(coordinates.count))
 ```
 ...
 Annotation models</a> example to learn how to add an `TMGLPolyine` object to
 your map.
 */
TMGL_EXPORT
@interface TMGLPolyline : TMGLMultiPoint <TMGLOverlay>

/**
 Creates and returns an `TMGLPolyline` object from the specified set of
 coordinates.
...
/**
 An `TMGLMultiPolyline` object represents a shape consisting of one or more
 ...
 reservation): each carriageway would be a distinct `TMGLPolyline` object.
 ...
 `TMGLVectorTileSource`’s multipolylines collectively using an
 `TMGLLineStyleLayer` or `TMGLSymbolStyleLayer` object.

 You cannot add an `TMGLMultiPolyline` object directly to a map view using
 ...
 */
TMGL_EXPORT
@interface TMGLMultiPolyline : TMGLShape <TMGLOverlay>

/**
 An array of polygons forming the multipolyline.
 */
@property (nonatomic, copy, readonly) NSArray<TMGLPolyline *> *polylines;
 ...
 */
+ (instancetype)multiPolylineWithPolylines:(NSArray<TMGLPolyline *> *)polylines;
