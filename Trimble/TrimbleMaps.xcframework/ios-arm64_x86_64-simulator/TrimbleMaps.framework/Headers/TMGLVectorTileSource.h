...
/**
 `TMGLVectorTileSource` is a map content source that supplies tiled vector data
 ...
 A vector tile source is added to an `TMGLStyle` object along with one or more
 `TMGLVectorStyleLayer` objects. A vector style layer defines the appearance of
 any content supplied by the vector tile source.
 
 `TMGLVectorTileSource` is optimized for data sets that are too large to fit
 ...
 <a href="http://geojson.org/">GeoJSON</a> data, use the `TMGLShapeSource`
 class. For tiled data that changes dynamically, the `TMGLComputedShapeSource`
 ...
 `TMGLVectorTileSource` object that you can use to initialize new style layers.
 ...
 ```swift
 let source = TMGLVectorTileSource(identifier: "pois", tileURLTemplates: ["https://example.com/vector-tiles/{z}/{x}/{y}.mvt"], options: [
     .minimumZoomLevel: 9,
     .maximumZoomLevel: 16,
     .attributionInfos: [
         TMGLAttributionInfo(title: NSAttributedString(string: "© Mapbox"), url: URL(string: "https://mapbox.com"))
     ]
 ])
 ...
 an `TMGLVectorTileSource` object.
 */
TMGL_EXPORT
@interface TMGLVectorTileSource : TMGLTileSource
...
/**
 ...
 source loads a feature if the source is added to an `TMGLMapView`’s style; that
 ...
 */
- (NSArray<id <TMGLFeature>> *)featuresInSourceLayersWithIdentifiers:(NSSet<NSString *> *)sourceLayerIdentifiers predicate:(nullable NSPredicate *)predicate NS_SWIFT_NAME(features(sourceLayerIdentifiers:predicate:));
